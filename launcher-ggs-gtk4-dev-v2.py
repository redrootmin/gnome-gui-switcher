...
class ScriptLauncherApp(Gtk.Application):
    ...
    def do_activate(self):
        ...
        # Отключаем дублирование кнопок окна
    ...
    def refresh_scripts(self, button=None):
        ...
        try:
            ...
            if not script_files:
                self.status_label.set_label("No scripts found in 'scripts' directory")
                return
            else:
                self.status_label.set_label(f"Found {len(script_files)} scripts")
        except FileNotFoundError:
            self.status_label.set_label("'scripts' directory not found")
            return
        ...
        try:
            ...
            if not icon_files:
                self.status_label.set_label(f"Found scripts but no icons in 'icons' directory")
        except FileNotFoundError:
            self.status_label.set_label(f"Found scripts but 'icons' directory not found")
        ...
        for script in script_files:
            ...
            if os.path.exists(icon_path):
                try:
                    ...
                    print(f"Successfully loaded icon: {icon_path}")  # Debug output
                except Exception as e:
                    print(f"Error loading icon {icon_path}: {e}")  # Debug output
                    icon = Gtk.Image.new_from_icon_name("image-missing-symbolic")
            else:
                print(f"Icon not found: {icon_path}")  # Debug output
                icon = Gtk.Image.new_from_icon_name("image-missing-symbolic")
    ...
    def on_script_clicked(self, button, script):
        ...
        if not os.path.exists(script_path):
            self.show_error_dialog(f"Script not found: {script}")
            return
        
        if not os.access(script_path, os.X_OK):
            self.show_error_dialog(f"Script is not executable: {script}")
            return
    ...
    def show_progress_dialog(self, script_path):
        dialog = Gtk.Dialog(transient_for=self.win, modal=True)
        dialog.set_title("Running Script")
        dialog.set_default_size(300, 100)
        
        # Remove default titlebar buttons to avoid duplication
